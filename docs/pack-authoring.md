...
**Under-declare, never over-declare.** A capability you claim but haven't verified against real hardware is worse than an honest gap — the predecessor's own bloat package list was found to mix fabricated entries with real ones (see `.claude/memory/gotcha_unverified_package_lists.md`) precisely because nobody could tell "declared" from "verified."
...
The reason is concrete, not stylistic: `pm disable-user` silently no-ops on Fire OS 5.x, and toybox's `tar -z` produces truncated archives on that build — both documented in `.claude/memory/gotcha_pm_disable_by_fireos_version.md` and `gotcha_toybox_tar_gzip_truncation.md`. These are **Amazon's bugs**, not Android's. If `ShieldPack` inherited `FireTvPack`, it would inherit both workarounds — including a two-step `tar cf` + `gzip` dance that costs real time on a large profile — for bugs it may never have. Compose the shared collaborator; declare quirks as data scoped to the pack that needs them (`AndroidQuirks.from_mapping`, per-pack `data/quirks.yml`).
...
- ADB-specific gotchas any pack composing `packs/android` inherits: `.claude/memory/gotcha_adb_shell_push_broken.md`, `gotcha_toybox_tar_gzip_truncation.md`, `gotcha_pm_disable_by_fireos_version.md`
